...
class WatsonxClient:
    ...
    def generate(self, user_message: str, context: str = "", history: Optional[list] = None) -> str:
        ...
        try:
            ...
            if "429" in err_str or "consumption_limit_reached" in err_str.lower() or "rate limit" in err_str.lower():
                return (
                    "⚠️ **watsonx.ai is currently rate-limited.**\n\n"
                    "IBM Cloud has reached its free request limit for this model, so the real AI response could not be generated. "
                    "The app is using a built-in fallback recipe response instead.\n\n"
                    "You can try again later or add a paid plan / new API credentials.\n\n"
                ) + self._demo_response(user_message, context)
            return (
    ...
    def _demo_response(self, user_message: str, context: str) -> str:
        ...
        elif any(w in msg for w in ["eggroll", "egg roll", "eggrolls", "egg rolls"]):
            return (
                "🥟 **Egg Rolls (Vegetable Egg Rolls)**\n\n"
                "**Quick recipe idea:**\n"
                "1. Mix shredded cabbage, carrots, onions, garlic, and a little soy sauce.\n"
                "2. Spoon the filling onto egg roll wrappers and roll tightly.\n"
                "3. Seal the edges with water or flour paste.\n"
                "4. Deep fry or air fry until crisp and golden.\n"
                "5. Serve with sweet chili sauce or spicy soy dip.\n\n"
                "💡 **Tip:** For a vegetarian version, use tofu or mushrooms instead of meat.\n\n"
                "*(Demo mode — set IBM_API_KEY in .env for full Granite AI responses)*"
            )
