...
async function analyzeNutrition() {
  ...
  try {
    ...
    const data = await res.json();
    if (data.error) throw new Error(data.error);
    DOM.nutritionModalResult.innerHTML = marked.parse(data.response || "No information found.");
  } catch (err) {
...
function parseAndDisplayNutrition(responseText) {
  // Attempt to extract nutrition numbers from AI response text
  const calorieMatch  = responseText.match(/(\d{2,4})\s*(?:cal|kcal|calories)/i);
  const proteinMatch  = responseText.match(/(\d+\.?\d*)\s*g\s*protein/i);
  const carbsMatch    = responseText.match(/(\d+\.?\d*)\s*g\s*carbs?/i);
  const fatMatch      = responseText.match(/(\d+\.?\d*)\s*g\s*fat/i);
  const fiberMatch    = responseText.match(/(\d+\.?\d*)\s*g\s*fiber/i);
  ...
  if (calorieMatch || proteinMatch) {
    ...
    const total    = protein + carbs + fat || 1; // avoid div/0

    DOM.calorieDisplay.textContent  = calories || "—";
    DOM.proteinVal.textContent      = protein  ? protein  + "g" : "—";
    DOM.carbsVal.textContent        = carbs    ? carbs    + "g" : "—";
    DOM.fatVal.textContent          = fat      ? fat      + "g" : "—";
    DOM.fiberVal.textContent        = fiber    ? fiber    + "g" : "—";

    // Progress bars — relative to total macros
    DOM.proteinBar.style.width = Math.min(100, Math.round(protein / total * 100)) + "%";
    DOM.carbsBar.style.width   = Math.min(100, Math.round(carbs   / total * 100)) + "%";
    DOM.fatBar.style.width     = Math.min(100, Math.round(fat     / total * 100)) + "%";
    DOM.fiberBar.style.width   = Math.min(100, Math.round(fiber   / total * 100)) + "%";
  }
}
